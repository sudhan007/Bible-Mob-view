...
export function ManageBook() {
  ...
  const columns = [
    {
      ...
      flex: 1,
      renderCell: (params) => (
        <div style={{ cursor : 'pointer' } } onClick={()=>{ navigete('/admin/manage-booksdetail/'+params.row._id) }} >
            <p >{params.row.bookTitle}</p>
        </div >
      )
    },
    {
      ...
      renderCell: (params) => (
        <div>
          <img style={{ width: 60 }} src={params.row.image} />
        </div>
      ),
    },
    {
  ...
  const addQuestionType = async () => {
    ...
    formData.append(`id`, formState.questionType);
    // setIsOpen(true);
    let responce = await axios
      .post(`${BaseUrluser}/addbook`, formData, {
        headers: {
